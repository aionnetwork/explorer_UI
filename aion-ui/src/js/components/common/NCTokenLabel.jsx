...
export default class NCEntityLabel extends Component
{
  ...
  render() {
    ...
    let { entityType, entityDescription, entitySymbol, entityName, entityId, linkActive=true, className } = this.props;
    ...
    if(entityDescription != null && entityDescription != ""){
      displayDescription = entityDescription;
    }
    if(entitySymbol != null && entitySymbol != ""){
      displaySym = " (" + entitySymbol.substring(0, 5) + ") ";
    }
    ...
    return( 
        <span 
          ...
          <span className=" dtext pt-text-overflow-ellipsis">{ displayDescription }</span>
        </span>
    );
  }
}
